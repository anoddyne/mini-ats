...
export default function VacancyDetailPage() {
  ...
  const getExperienceLabel = (level) => {
    switch (level) {
      case 'NO_EXPERIENCE': return 'Нет опыта';
      case 'JUNIOR': return 'Junior (до 1 года)';
      case 'MIDDLE': return 'Middle (1-3 года)';
      case 'SENIOR': return 'Senior (3-6 лет)';
      case 'LEAD': return 'Lead (6+ лет)';
          ...
          <div className="p-6">
            <div className="flex justify-between items-start flex-wrap gap-2">
              <div>
                ...
                {vacancy.companyName && (
                    <p className="text-gray-600 mt-1">🏢 {vacancy.companyName}</p>
                )}
              </div>
              {statusInfo && (
            ...
            <div className="mt-6">
              ...
              <p className="text-gray-700 whitespace-pre-wrap mt-1">{vacancy.description || 'Описание отсутствует'}</p>
            </div>
            ...
            <div className="mt-4">
              ...
              <p className="text-gray-700">{getExperienceLabel(vacancy.experienceLevel)}</p>
            </div>

            {/* Кнопка отклика - только для кандидата и открытой вакансии */}
            ...
            {user?.role === 'RECRUITER' && (vacancy.status === 'OPEN' || vacancy.status === 'DRAFT') && (
                <div className="mt-6 flex gap-3">
                  <button
                      onClick={() => navigate(`/recruiter/vacancies/${vacancy.vacancyId}/edit`)}
                      ...
                      <button
                          onClick={async () => {
                            if (confirm('Закрыть вакансию? Отклики больше не будут приниматься.')) {
                              await vacancyAPI.close(vacancy.vacancyId);
                              await loadVacancy();
                            }
                          }}
